...
void CScriptView::UpdateCaret()
{
    ...
    GetDocument()->UpdateAllViewsAndNonViews(this, VIEWUPDATEHINT_SCRIPTPOSCHANGED, NULL);
}
...
DWORD CScriptView::ParseLine(DWORD dwCookie, int nLineIndex, TEXTBLOCK *pBuf, int &nActualItems)
{
	...
	for (; ; I++)
	{
        ...
        if (theApp._fAllowBraceSyntax && pszChars[I] == '{')
        {
...
void CScriptView::OnMouseMove(UINT nFlags, CPoint point)
{
    if (_pwndToolTip)
    {
        ...
        {
            _pwndToolTip->Hide();
            SetTimer(TOOLTIPTIMER_ID, TOOLTIPTIMER_INITIAL, NULL);
        }
    }
    _ptLastMM = point;
...
void CScriptView::_OnAddAs(Vocab000::WordClass dwClass)
{
    Vocab000 *pVocab000 = theApp.GetResourceMap().GetVocab000();
    if (pVocab000)
    {
        HRESULT hr = pVocab000->AddNewWord(_contextMenuText, dwClass, TRUE);
        if (SUCCEEDED(hr))
        {
            pVocab000->QuickSave();
        }
...
void CScriptView::OnAddAsSynonymOf()
{
    ...
    if (IDOK == dialog.DoModal())
    {
        ...
        Vocab000 *pVocab000 = theApp.GetResourceMap().GetVocab000();
        if (SUCCEEDED(pVocab000->AddSynonym(strWord, strSynonym)))
        {
            pVocab000->QuickSave();
        }
    }
...
void CScriptView::OnGotoScriptHeader()
{
    ...
    theApp.OpenScriptHeader((PCSTR)_gotoScriptText);
}
...
void CScriptView::OnGotoDefinition()
{
    ...
    if (pDoc)
    {
        ...
        theApp.OpenScriptAtLine(_gotoScript.GetFullPath(), _gotoLineNumber + 1);
    }
}
...
ToolTipResult CScriptView::_DoToolTipParse(CPoint pt)
{
    ...
    if (theApp.AreHoverTipsEnabled())
    {
        ...
        timer.Start();
        g_Parser.Parse(script, stream, NULL); // REVIEW: do we need context???
        char sz[100];
    ...
    if (_pwndToolTip)
    {
        ...
        if (fInEditableArea)
        {
            ...
            if (!result.empty())
            {
                ...
                SetTimer(TOOLTIPEXPIRE_ID, TOOLTIPEXPIRE_TIMEOUT, NULL);
            }
        }
    }
}
...
void CScriptView::OnIntellisense()
{
    ...
    if (!theApp.IsCodeCompletionEnabled() && !theApp.IsParamInfoEnabled())
    {
        // Nothing to see here...
        return;
    }
...
/*
    ...
    if (WAIT_OBJECT_0 == WaitForSingleObject(_hEventACDone, INFINITE))
    {
        ...
        if (theApp.IsCodeCompletionEnabled())
        {
...
{
    map<WORD, CSCOFile>::iterator it = _mapScriptToObject.find(wScript);
...
bool ObjectFileScriptLookups::_LoadSCOFile(WORD wScript)
{
    ...
    std::string filename = theApp.GetResourceMap().GetScriptObjectFileName(wScript);
    if (!filename.empty())
    {
